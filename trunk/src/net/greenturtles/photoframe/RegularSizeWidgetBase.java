...
public abstract class RegularSizeWidgetBase extends AppWidgetProvider {
    ...
    private void onUpdateOne(Context context, AppWidgetManager appWidgetManager, int appWidgetId) {
        SharedPreferences config = context.getSharedPreferences(ConfigurationActivityBase.PREFS_NAME, 0);

        // 准备layout对象
    	int layoutResId;
        String frameTypeKey = String.format(ConfigurationActivityBase.PREFS_FOLDER_FRAME_TYPE_FIELD_PATTERN, appWidgetId);
        ...
        // 读取当前图片index
        String currentIndexKey = String.format(ConfigurationActivityBase.PREFS_UPDATE_CURRENT_PIC_INDEX_FIELD_PATTERN, appWidgetId);
        int currentIndex = config.getInt(currentIndexKey, -1);
        
        // 读取文件列表
        String picFolderKey = String.format(ConfigurationActivityBase.PREFS_FOLDER_PATH_FIELD_PATTERN, appWidgetId);
    ...
    public void onDeleted(Context context, int[] appWidgetIds) {
        ...
        for (int appWidgetId : appWidgetIds) {
            ...
            configEditor.remove(String.format(ConfigurationActivityBase.PREFS_UPDATE_RATE_FIELD_PATTERN, appWidgetId));
            configEditor.remove(String.format(ConfigurationActivityBase.PREFS_FOLDER_PATH_FIELD_PATTERN, appWidgetId));
            configEditor.remove(String.format(ConfigurationActivityBase.PREFS_FOLDER_FRAME_TYPE_FIELD_PATTERN, appWidgetId));
            configEditor.remove(String.format(ConfigurationActivityBase.PREFS_UPDATE_CURRENT_PIC_INDEX_FIELD_PATTERN, appWidgetId));
            configEditor.commit();
        }
    ...
    private void registerAlarmService(Context context, int appWidgetId) {
        ...
        // time for
        SharedPreferences config = context.getSharedPreferences(ConfigurationActivityBase.PREFS_NAME, 0);
        int updateRateSeconds = config.getInt(String.format(ConfigurationActivityBase.PREFS_UPDATE_RATE_FIELD_PATTERN, appWidgetId), -1);
